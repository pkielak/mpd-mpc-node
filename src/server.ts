  ...
  try {
    ...
    switch (resource) {
      ...
      case "current-song":
        ...
        text = JSON.stringify(data || { message: "No song playing" }, null, 2);
        break;
  ...
  switch (request.params.name) {
    ...
    case "playlist_manager": {
      ...
      try {
        ...
        throw new Error(`Error managing playlist: ${(error as Error).message}`);
      }
    }
...
export async function ensureConnected(): Promise<void> {
  if (!(mpdClient as any).connected) {
    try {
      ...
      console.log(`Connected to MPD server at ${MPD_HOST}:${MPD_PORT}`);
    } catch (error) {
...
async function main() {
  try {
    // Try to connect to MPD server at startup
    await mpdClient.connect();
    console.log(`Connected to MPD server at ${MPD_HOST}:${MPD_PORT}`);
